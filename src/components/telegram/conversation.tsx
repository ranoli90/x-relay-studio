    ...
    <div className="flex h-full min-h-0 flex-col bg-[var(--tg-bg)]">
      <header className="flex h-14 shrink-0 items-center gap-2 border-b border-white/5 px-2">
        ...
        >
          ...
          <span className="min-w-0">
            <span className="block truncate text-[16px] font-medium text-[var(--tg-text)]">
              {chat.title}
            </span>
            <span className="block truncate text-[12px] text-[var(--tg-text-secondary)]">
              {chat.kind === "notes" ? "Saved in this studio" : "Telegram"}
            </span>
          </span>
        </button>
      </header>
      <div ref={scroller} className="min-h-0 flex-1 overflow-y-auto px-3 py-3">
        {loading ? (
          ...
          <p className="py-8 text-center text-sm text-[var(--tg-text-secondary)]">No messages yet.</p>
        ) : (
          messages.map((msg, i) => {
            ...
            return (
              <div key={msg.id}>
                {dayBreak ? (
                  <p className="my-3 text-center text-[12px] text-[var(--tg-text-secondary)]">
                ...
                <div className={cn("mb-1.5 flex", msg.fromSelf ? "justify-end" : "justify-start")}>
                  <div
                    className={cn(
                      "max-w-[min(100%,28rem)] rounded-2xl px-3 py-2 text-[15px] leading-snug",
                      msg.fromSelf
                        ? "rounded-br-md bg-[var(--tg-own-bubble)] text-white"
                        : "rounded-bl-md bg-[var(--tg-item-hover)] text-[var(--tg-text)]",
                    )}
                  >
                    {!msg.fromSelf ? (
                      <p className="mb-0.5 text-[12px] font-medium text-[var(--tg-primary)]">
                        {msg.authorName}
                      </p>
                    ) : null}
                    ...
                    <p
                      className={cn(
                        ...
                        msg.fromSelf ? "text-white/70" : "text-[var(--tg-text-secondary)]",
                      )}
                    >
      ...
      <Composer disabled={sending || chat.kind !== "notes"} onSend={onSend} />
    </div>
  );
}
