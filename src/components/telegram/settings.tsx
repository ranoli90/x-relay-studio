...
import { publicEffectiveLabel, type AutoReplyView } from "@/lib/operator/effective-state";
...
export function SettingsPane({
  ...
  const [effective, setEffective] = useState<AutoReplyView | null>(null);
    ...
    void Promise.all([loadDesk().catch(() => null), loadOperatorDeskFn().catch(() => null)]).then(
      ([deskRaw, op]) => {
        ...
        if (op) {
          ...
          if (op.effective) setEffective(op.effective);
        }
      },
    );
    return () => {
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-8">
        {effective ? (
          <section className="rounded-xl bg-[var(--tg-item-hover)] p-4">
            <h3 className="text-sm font-medium">Effective auto-reply</h3>
            <p className="mt-2 text-sm">{publicEffectiveLabel(effective)}</p>
            <p className="mt-2 text-sm leading-relaxed text-[var(--tg-text-secondary)]">{effective.reason}</p>
            {effective.recovery ? (
              <p className="mt-2 text-sm text-[var(--tg-text-secondary)]">{effective.recovery}</p>
            ) : null}
            <p className="mt-2 font-mono text-[11px] text-[var(--tg-text-secondary)]">
              Desired {effective.desired ? "on" : "off"} · {effective.scope}
            </p>
          </section>
        ) : null}
        <section className={`${effective ? "mt-3 " : ""}rounded-xl bg-[var(--tg-item-hover)] p-4`}>
