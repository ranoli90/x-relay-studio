...
/** Isolated workspace: no site.json, no public/og.jpg. Product identity must not leak. */
const EMPTY_CWD = mkdtempSync(join(tmpdir(), "grok-og-blank-"));
...
test("this product injects X Relay identity from site.json", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    host: "x-relay.grok.me",
    cwd: TEMPLATE_ROOT,
  });
  assert.match(out, /property="og:title" content="X Relay"/);
  assert.match(out, /property="og:image" content="https:\/\/x-relay\.grok\.me\/og\.jpg"/);
});
