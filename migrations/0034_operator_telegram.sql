-- Operator Telegram assistant: permissions, business publication, money, media, outbox.
-- Additive. Rollback: stop using the new tables, set processing_permission=false,
-- desired_auto_reply=false. Do not re-enable auto-send by rolling the code back
-- onto a desk that was stopped.
...
alter table agent_personas
  add column if not exists desired_auto_reply boolean not null default false;
alter table agent_personas
  add column if not exists ai_disclosed boolean not null default true;
alter table agent_personas
  add column if not exists writer_last_ok boolean;
alter table agent_personas
  add column if not exists writer_last_at timestamptz;
...
alter table agent_offers
  add column if not exists offer_revision text;
...
create unique index if not exists business_offers_service_rev
  on business_offers (revision_id, service_key);
...
create table if not exists payment_evidence (
  ...
  currency text not null,
  destination_id text not null,
  provenance text not null default 'evidence_candidate',
  status text not null,
  created_at timestamptz not null default now()
);
...
create index if not exists memory_facts_customer_idx
  on memory_facts (user_id, customer_id, status);
